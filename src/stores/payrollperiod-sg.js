...
export const usePayrollPeriodSGStore = defineStore("sgPayrollPeriodStore", () => {
    ...
    const getList = async() => {
        try{
            const { data } = await getFN(`api/timekeeping/sg-payroll-period/list`,null);
            list.value = data;
        }catch(error){
    ...
    const getData = async() => {
        try{
            const { data } = await getFN(`api/timekeeping/sg-payroll-period/index`,null);
    ...
    const prepareDTR = async (period_id) => {
        const { data,message,status } = await postFN(`api/timekeeping/sg-payroll-period/prepare`,{ period_id : period_id });
